@foreach ($posts as $posts)
             <div class="frame">
				...
				<a href="{{ $posts->slug }}">
					...
					<img src= "{{ Storage::url($posts->image)}}" alt="{{$posts->image}}" class="img">
					@if($posts->ads === 1)
					<div class="fdetails"><span class="ads">AD</span></div>
					@else
					<div class="fdetails"><i class="fa fa-heart-o" aria-hidden="true"></i><i class="fa fa-plus" aria-hidden="true"></i><span><i class="fa fa-arrow-down" aria-hidden="true"></i></span></div>
					@endif
				</a>
             </div>
			@endforeach